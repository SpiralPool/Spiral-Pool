...
# =============================================================================
# Spiral Stratum — DigiByte (DGB) Regtest Integration Test
# =============================================================================
#
# WHAT THIS DOES:
#   Tests the FULL block lifecycle end-to-end against a real DigiByte daemon
...
#
#   Flow:
#     digibyted (regtest) --> pool (stratum) --> cpuminer --> block found!
#          |                      |                               |
#          |<---- submitblock ----|                               |
#          |---- ZMQ hashblock -->|                               |
#          |                      |---- DB insert + status ------>|
...
#
# PREREQUISITES:
#   1. DigiByte Core installed (digibyted + digibyte-cli in PATH)
#      - Build from: https://github.com/DigiByte-Core/digibyte
...
#   chmod +x scripts/linux/regtest-dgb.sh
#   ./scripts/linux/regtest-dgb.sh
#
#   # Custom binary paths:
#   DIGIBYTED=/opt/dgb/bin/digibyted \
#   DIGIBYTECLI=/opt/dgb/bin/digibyte-cli \
...
#               logs/regtest/digibyted-regtest.log     (daemon)
#               logs/regtest/digibyted-startup.log     (daemon stderr)
#
# EXIT CODES:
...
#   Regtest blockchain data is in ~/.digibyte/regtest/ (delete to reset).
#
# =============================================================================
set -euo pipefail
...
CONFIG_FILE="$PROJECT_ROOT/config/regtest/config-dgb-regtest.yaml"
POOL_BINARY="$PROJECT_ROOT/src/stratum/spiralpool"
LOG_DIR="$PROJECT_ROOT/logs/regtest"

# DigiByte Core binaries
# install.sh creates lowercase symlinks in /usr/local/bin: digibyted, digibyte-cli
# Override: DIGIBYTED=/opt/spiralpool/dgb/bin/digibyted ./regtest-dgb.sh
DIGIBYTED="${DIGIBYTED:-digibyted}"
DIGIBYTECLI="${DIGIBYTECLI:-digibyte-cli}"

# CPU miner binary
# Override: CPUMINER=/path/to/minerd ./regtest-dgb.sh
CPUMINER="${CPUMINER:-minerd}"

# Database credentials (must match config-dgb-regtest.yaml)
DB_NAME="${DB_NAME:-spiralstratum_dgb_regtest}"
...
# Daemon RPC credentials (must match config-dgb-regtest.yaml)
RPC_USER="${RPC_USER:-spiraltest}"
RPC_PASS="${RPC_PASS:-spiraltest}"
RPC_PORT="${RPC_PORT:-18543}"       # DGB regtest RPC port
P2P_PORT="${P2P_PORT:-18544}"       # DGB regtest P2P port
ZMQ_PORT="${ZMQ_PORT:-29340}"       # ZMQ pub port for block notifications

# Pool stratum port (must match config-dgb-regtest.yaml)
STRATUM_PORT="${STRATUM_PORT:-16335}"

# Test parameters
TEST_BLOCKS="${TEST_BLOCKS:-5}"  # How many blocks to mine (override: TEST_BLOCKS=150 ./regtest-dgb.sh)
...
# Coinbase maturity: DGB inherits Bitcoin's 100-block rule. The miner keeps running
...
# Pool API port (must match config-dgb-regtest.yaml api_port)
API_PORT="${API_PORT:-14002}"
...
# Helper: run digibyte-cli with standard regtest flags
dgbcli() {
    "$DIGIBYTECLI" -regtest -rpcuser="$RPC_USER" -rpcpassword="$RPC_PASS" -rpcport="$RPC_PORT" "$@"
}

# Helper: run digibyte-cli with wallet
dgbcli_wallet() {
    dgbcli -rpcwallet="regtest-pool-dgb" "$@"
}
...
cleanup() {
    ...
    dgbcli stop 2>/dev/null || true
    pkill -9 -f "digibyted.*regtest" 2>/dev/null || true
    ...
    rm -f "$LOG_DIR/config-dgb-regtest-ha.yaml" 2>/dev/null || true
...
check_binary() {
    ...
    if ! command -v "$bin" &>/dev/null; then
        ...
        log_info "  Or override: ${name^^}=/path/to/$bin ./regtest-dgb.sh"
        return 1
    fi
...
wait_for_rpc() {
    local max_wait=30
    local waited=0
    log_info "Waiting for digibyted RPC on port $RPC_PORT..."
    while ! dgbcli getblockchaininfo &>/dev/null; do
        sleep 1
        waited=$((waited + 1))
        if [[ $waited -ge $max_wait ]]; then
            log_error "digibyted RPC not responding after ${max_wait}s"
            log_error "Check: $LOG_DIR/digibyted-startup.log"
            return 1
        fi
    done
    log_ok "digibyted RPC ready (${waited}s)"
}
...
get_block_count() {
    dgbcli getblockcount 2>/dev/null || echo "0"
}
...
check_binary "digibyted"    "$DIGIBYTED"   "Build from https://github.com/DigiByte-Core/digibyte"
check_binary "digibyte-cli" "$DIGIBYTECLI" "(included with DigiByte Core)"
...
# =============================================================================
# STEP 2: Start DigiByte Daemon (regtest mode)
# =============================================================================

log_step "Step 2/10: Start digibyted (regtest)"
...
log_info "Starting digibyted with:"
...
log_info "  Log:        $LOG_DIR/digibyted-regtest.log"
...
"$DIGIBYTED" \
    ...
    -blockfilterindex=1 \
    -printtoconsole=0 \
    -debuglogfile="$LOG_DIR/digibyted-regtest.log" \
    &>"$LOG_DIR/digibyted-startup.log" &

DAEMON_PID=$!
log_info "digibyted PID: $DAEMON_PID"

wait_for_rpc

# Show chain info
CHAIN_INFO=$(dgbcli getblockchaininfo 2>/dev/null)
...
log_info "Creating regtest wallet 'regtest-pool-dgb'..."
...
CREATE_OUT=$(dgbcli createwallet "regtest-pool-dgb" 2>&1) || \
LOAD_OUT=$(dgbcli loadwallet "regtest-pool-dgb" 2>&1) || true
...
MINING_ADDRESS=$(dgbcli_wallet getnewaddress "pool-coinbase" "bech32")
...
# DGB regtest has a non-standard difficulty schedule — difficulty jumps from regtest
...
if [[ "$CURRENT_HEIGHT" -eq 0 ]]; then
    ...
    dgbcli_wallet generatetoaddress 1 "$MINING_ADDRESS" &>/dev/null
    CURRENT_HEIGHT=$(get_block_count)
...
GBT_RESULT=$(dgbcli getblocktemplate '{"rules":["segwit"]}' 2>&1) || true
...
if psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c "SELECT 1;" &>/dev/null; then
    ...
    # The blockchain resets (rm -rf ~/.digibyte/regtest) but DB persists — old records
    ...
    if [[ ! -t 0 ]]; then
        ...
        log_error ""
        log_error "Then re-run: nohup ./scripts/linux/regtest-dgb.sh > regtest.log 2>&1 &"
        exit 1
    fi
...
log_info "Timeout: ${MINER_WAIT_SECS}s (DGB regtest diff=1, ~${EST_SECS_PER_BLOCK}s/block with $MINER_THREADS threads)"
echo ""
...
if [[ $BLOCKS_FOUND -lt $TEST_BLOCKS ]]; then
    ...
    log_error "  3. Check daemon responding: $DIGIBYTECLI -regtest -rpcuser=$RPC_USER -rpcpassword=$RPC_PASS -rpcport=$RPC_PORT getblockchaininfo"
...
if [[ "$MATURITY_WAIT_SECS" -gt 0 ]] && [[ $BLOCKS_FOUND -ge $TEST_BLOCKS ]]; then
    ...
    # on DGB regtest because it requires real PoW, unlike standard Bitcoin Core regtest).
    #
    ...
    while [[ $MATURITY_ELAPSED -lt $MATURITY_WAIT_SECS ]]; do
        ...
        DB_CONFIRMED=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c \
            "SELECT COUNT(*) FROM blocks_dgb_regtest WHERE status = 'confirmed';" 2>/dev/null | tr -d ' ' || echo "?")
        DB_PENDING=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c \
            "SELECT COUNT(*) FROM blocks_dgb_regtest WHERE status = 'pending';" 2>/dev/null | tr -d ' ' || echo "?")
        DB_ORPHANED=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c \
            "SELECT COUNT(*) FROM blocks_dgb_regtest WHERE status = 'orphaned';" 2>/dev/null | tr -d ' ' || echo "?")
    ...
    psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c \
        "SELECT blockheight, status, confirmationprogress, orphan_mismatch_count, stability_check_count FROM blocks_dgb_regtest ORDER BY blockheight;" 2>/dev/null || \
...
if [[ $BLOCKS_FOUND -ge $TEST_BLOCKS ]]; then
    ...
    IMMATURE_BALANCE="0"
    CHAIN_HEIGHT=$(get_block_count)
    WALLET_BALANCE=$(dgbcli_wallet getbalance 2>/dev/null || echo "0")
    IMMATURE_BALANCE=$(dgbcli_wallet getwalletinfo 2>/dev/null | grep -o '"immature_balance":[^,}]*' | head -1 | cut -d: -f2 | tr -d ' ') || true
    IMMATURE_BALANCE="${IMMATURE_BALANCE:-0}"

    if (( $(echo "$WALLET_BALANCE > 0" | bc -l 2>/dev/null || echo 0) )); then
        log_ok "[1/8] Block rewards mature and spendable: $WALLET_BALANCE DGB (height=$CHAIN_HEIGHT)"
        PAYMENT_PASS=$((PAYMENT_PASS + 1))
        COINBASE_MATURE=1
    elif (( $(echo "$IMMATURE_BALANCE > 0" | bc -l 2>/dev/null || echo 0) )); then
        log_ok "[1/8] Block rewards received (immature: $IMMATURE_BALANCE DGB, height=$CHAIN_HEIGHT)"
        PAYMENT_PASS=$((PAYMENT_PASS + 1))
    else
        ...
        while [[ $REWARD_WAIT -lt 120 ]]; do
            ...
            if (( $(echo "$IMMATURE_BALANCE > 0" | bc -l 2>/dev/null || echo 0) )); then
                log_ok "[1/8] Block rewards received (immature: $IMMATURE_BALANCE DGB)"
                PAYMENT_PASS=$((PAYMENT_PASS + 1))
                break
            fi
            log_info "  ${REWARD_WAIT}s — immature: $IMMATURE_BALANCE DGB (waiting for wallet to detect coinbase)"
        done
    fi
    ...
    BLOCK_HASH=$(dgbcli getblockhash "$FIRST_POOL_BLOCK" 2>/dev/null) || true
    if [[ -n "$BLOCK_HASH" ]] && [[ ${#BLOCK_HASH} -eq 64 ]]; then
        BLOCK_JSON=$(dgbcli getblock "$BLOCK_HASH" 2 2>/dev/null) || true
        ...
        if [[ -n "$COINBASE_ADDR" ]]; then
            ...
            ADDR_MINE=$(dgbcli_wallet getaddressinfo "$COINBASE_ADDR" 2>/dev/null | grep '"ismine"' | head -1 | grep -o 'true\|false') || true
            if [[ "$ADDR_MINE" == "true" ]]; then
                log_ok "[2/8] SOLO reward verified — coinbase ${COINBASE_VALUE:-?} DGB to wallet address $COINBASE_ADDR (ismine=true)"
                PAYMENT_PASS=$((PAYMENT_PASS + 1))
            else
    ...
    # DGB inherits Bitcoin's 100-block coinbase maturity rule.
    ...
    if [[ $COINBASE_MATURE -eq 0 ]] && [[ "$COINBASE_WAIT_SECS" -gt 0 ]]; then
        ...
        while [[ $MATURITY_ELAPSED -lt $COINBASE_WAIT_SECS ]]; do
            ...
            if (( $(echo "$WALLET_BALANCE > 0" | bc -l 2>/dev/null || echo 0) )); then
                log_ok "Coinbase matured! Balance: $WALLET_BALANCE DGB (height=$CHAIN_HEIGHT)"
                COINBASE_MATURE=1
                break
            fi

            if [[ $((MATURITY_ELAPSED % 60)) -eq 0 ]]; then
                log_info "  ${MATURITY_ELAPSED}s — height: $CHAIN_HEIGHT, confirms: ~$FIRST_CONFIRMS/100, balance: $WALLET_BALANCE DGB"
            fi
            sleep 30
    ...
    if [[ $COINBASE_MATURE -eq 1 ]]; then
        ...
        PAYOUT_ADDRESS=$(dgbcli_wallet getnewaddress "test-payout" "bech32" 2>/dev/null) || true
        ...
        PAYMENT_TXID=$(dgbcli_wallet sendtoaddress "$PAYOUT_ADDRESS" 1.0 2>&1) || true
        if [[ ${#PAYMENT_TXID} -eq 64 ]]; then
            log_ok "[4/8] Payment sent: txid=$PAYMENT_TXID"
            PAYMENT_PASS=$((PAYMENT_PASS + 1))

            TX_INFO=$(dgbcli_wallet gettransaction "$PAYMENT_TXID" 2>/dev/null) || true
    ...
    FIRST_HASH=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c \
        "SELECT hash FROM blocks_dgb_regtest WHERE blockheight = $FIRST_POOL_BLOCK AND status = 'confirmed' LIMIT 1;" 2>/dev/null | tr -d ' ') || true

    if [[ -n "$FIRST_HASH" ]]; then
        PAID_RESULT=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c \
            "UPDATE blocks_dgb_regtest SET status = 'paid', confirmationprogress = 1.0
        ...
        REVERT_RESULT=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -c \
            "UPDATE blocks_dgb_regtest SET status = 'pending'
             WHERE blockheight = $FIRST_POOL_BLOCK AND hash = '$FIRST_HASH'
    ...
    psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -q -c \
        "INSERT INTO payments (poolid, coin, address, amount, transactionconfirmationdata, created)
         VALUES ('dgb_regtest', 'DGB', '$PAY_ADDR_FOR_DB', $PAY_AMT_FOR_DB, '$PAY_TXID_FOR_DB', NOW());" 2>/dev/null || true

    PAY_COUNT=$(psql -h "$DB_HOST" -p "$DB_PORT" -U "$DB_USER" -d "$DB_NAME" -t -c \
        "SELECT COUNT(*) FROM payments WHERE poolid = 'dgb_regtest' AND transactionconfirmationdata = '$PAY_TXID_FOR_DB';" 2>/dev/null | tr -d ' ') || echo "0"
    ...
    if [[ $COINBASE_MATURE -eq 0 ]]; then
        ...
        if [[ "$COINBASE_WAIT_SECS" -eq 0 ]]; then
            log_info "  For full 8/8: COINBASE_WAIT_SECS=7200 ./scripts/linux/regtest-dgb.sh"
        fi
    else
...
if [[ $BLOCKS_FOUND -ge $TEST_BLOCKS ]]; then
    ...
    HA_STRATUM_PORT=16336
    HA_API_PORT=14003
    HA_METRICS_PORT=19103
    HA_CONFIG="$LOG_DIR/config-dgb-regtest-ha.yaml"
...
if [[ $BLOCKS_FOUND -ge $TEST_BLOCKS ]]; then
    ...
    if ! dgbcli getblockchaininfo &>/dev/null; then
    ...
    dgbcli loadwallet "regtest-pool-dgb" &>/dev/null || true
...
FINAL_BALANCE=$(dgbcli_wallet getbalance 2>/dev/null || echo "unknown")
...
echo "  Wallet balance:        $FINAL_BALANCE DGB"
...
echo "  Daemon:   $LOG_DIR/digibyted-regtest.log"
echo "  Startup:  $LOG_DIR/digibyted-startup.log"
echo ""
...
echo ""
echo -e "  ${CYAN}To re-run: ./scripts/linux/regtest-dgb.sh${NC}"
echo -e "  ${CYAN}To reset regtest chain: rm -rf ~/.digibyte/regtest${NC}"
echo ""
