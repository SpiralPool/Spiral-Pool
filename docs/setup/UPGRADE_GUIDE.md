# Upgrading to Spiral Pool v1.2.2 (Convergent Spiral)

## Is a full reinstall required?

**No. There are zero incompatibilities between any prior version (v1.0.0, v1.1.x) and v1.2.2 for any coin.**
...
- **Version string consistency** — all strings now semver `1.2.2`
...
The v1.0.0 → v1.1.0 changes are listed below. **None require a reinstall, OS change, config change, or manual migration.** The v1.1.x → v1.2.2 changes are also fully backward-compatible — no new database migrations, no config format changes.
...
| Version strings | `1.0.0 / BLACKICE` → `1.2.2 / CONVERGENT_SPIRAL` throughout | Cosmetic. |
...
The version line should show `1.2.2`. If Sentinel is running:

```bash
sudo journalctl -u spiralsentinel -n 20
```

Look for `Spiral Sentinel v1.2.2-CONVERGENT_SPIRAL` followed by `CONVERGENT SPIRAL EDITION` in the startup log.
